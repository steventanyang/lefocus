...
4. **Refresh island layouts** so compact and expanded states react intelligently to audio/timer combinations.
5. **Increase expanded real estate** (420×150) so artwork, waveform, controls, and timer can breathe.
...
* **Audio only:** 4-bar waveform on left, 18 px rounded thumbnail on right, timer hidden.
...
Waveform data now always draws with `startX = 18` so spacing stays consistent with the notch curve.

### 6.3 Expanded Layout

* **Artwork:** 40 px square positioned `(28, top-28)`, dark placeholder + white stroke when real art exists.
* **Metadata block:** Title (14 pt semibold) + artist (12 pt regular) stacked with 4 pt line spacing, pinned to top-left of audio column.
* **Waveform:** 4 bars, anchored just above the metadata block and to the right of artwork; opacity adapts to timer state.
* **Playback controls:** 42 px icons with 18 px spacing, left-aligned under the metadata column (no longer floating in the center when timers run).
* **Timer section:** Right half reserved for large 24 pt monospaced time and (when applicable) “End/Cancel” text buttons. When timer idle, this space remains empty, emphasizing artwork + controls.
