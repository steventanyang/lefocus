...
pub fn compute_phash(png_bytes: &[u8]) -> Result<String> {
    ...
    info!("pHash: Decoded {}×{} image ({} bytes) in {}ms", width, height, png_bytes.len(), decode_time_ms);
    ...
    let small_img = if width > 256 || height > 256 {
        ...
        let resized_buffer = image::imageops::resize(&img.to_rgba8(), new_width, new_height, image::imageops::FilterType::Triangle);
    ...
    if small_width < width || small_height < height {
        info!("pHash: Downscaled {}×{} → {}×{} in {}ms", width, height, small_width, small_height, downscale_time_ms);
    }
