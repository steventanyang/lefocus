...
/**
 * Get color for a given bundle ID with priority:
 * 1. Custom config color (from database)
 * 2. Hardcoded APP_COLORS
 * 3. Confidence-based color (if confidence provided)
 * 4. Default gray
 */
export function getAppColor(
  ...
  // Priority 1: Custom config color from database
  if (config?.color) {
    return config.color;
  }

  // Priority 2: Hardcoded APP_COLORS
  if (bundleId in APP_COLORS) {
    return APP_COLORS[bundleId];
  }

  // Priority 3: Confidence-based color if provided
  if (confidenceScore !== undefined) {
    return getConfidenceColor(confidenceScore);
  }
